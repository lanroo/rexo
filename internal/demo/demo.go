...
var pipeline = []stage{
	{
		id:     "summary",
		title:  "Summary",
		system: "You are a concise technical educator. Answer in the language of the topic.",
	...
	},
	{
		...
		system: "You are an instructional designer. Answer in the language of the topic.",
		build: func(topic string, prev map[string]string) string {
	...
	},
	{
		id:     "outline",
		title:  "Slide outline",
		system: "You are a curriculum designer. Answer in the language of the topic.",
		build: func(topic string, prev map[string]string) string {
	...
	},
	{
		id:     "quiz",
		title:  "Quiz question",
		system: "You are an assessment writer. Answer in the language of the topic.",
		build: func(topic string, prev map[string]string) string {
...
func Run(ctx context.Context, opt Options) (*Result, error) {
	...
	for _, st := range pipeline {
		req := providers.Request{System: st.system + concise, Prompt: st.build(topic, outputs)}
...
func writeLesson(projectDir, topic string, steps []StepResult) (string, error) {
	...
	fmt.Fprintf(&b, "# Mini-aula: %s\n\n", topic)
	provider := "unknown"
	if len(steps) > 0 {
		provider = steps[0].Provider
	}
	fmt.Fprintf(&b, "_Generated by REXO — capability `text.generate@1`, provider `%s`._\n\n", provider)
	for _, s := range steps {
